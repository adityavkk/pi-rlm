...
Terminal lifecycle metadata is accepted for deletion only when its `runId` matches both a strictly validated manifest and the sole authoritative terminal in the validated event journal, including the terminal status. `finish` applies the same check. Whether terminal metadata succeeds or fails, `finish` and `abandon` first release the in-process owner and then remove the active marker. If marker unlink fails, release atomically renames it to an owner-bound `.pi-rlm-inactive-<owner>.json` tombstone and syncs the directory; scanners never interpret that tombstone as live. If both unlink and fallback fail, the bounded metadata warning is returned and other processes conservatively retain any surviving active marker, but this process no longer classifies the run as owned. A thrown run without terminal evidence remains active/incomplete and is eligible only for abandoned-run handling after grace, never terminal retention.
...
Immediately before removal, cleanup reacquires the lifecycle claim, rereads lifecycle and lease state, revalidates root/run device and inode identity, and atomically renames the run to a random root-contained quarantine name. It verifies the same inode after rename and verifies its absence after recursive removal. A replacement at the old run name is therefore outside the removal target and survives. Only an inode removed by the current cleaner appears in `deleted`; a candidate already claimed or removed by another cleaner appears in `skipped` with `already_claimed` or `already_removed`. Missing nested lifecycle, manifest, or journal evidence while the top-level run still exists is a typed retained failure, never a deletion. Identity mismatch or removal failure retains the path or quarantine and returns a typed failure with bounded survivor metadata.
