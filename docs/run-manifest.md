...
Dynamic turn and extractor prompt text is not predicted in the manifest. The manifest binds its renderer version and canonical configuration. Each actual provider request records `promptSha256` on its `provider_attempted` journal event. Opaque external operations have no provider request and therefore omit that field.
