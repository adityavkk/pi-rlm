...
The cancellation case calls public `AgentSession.abort()` only after the pending controller provider starts. Pi propagates cancellation to the provider signal once, retains one accounting attempt and one `run_cancelled` terminal, and durably records the bounded cancelled `rlm_run` result in the current session. Pi 0.80.10 then makes one final host-model request containing that tool result; the script returns terminal assistant text and emits no second tool call. Releasing the provider's late-emission gate leaves the complete managed run tree, journal, accounting, and Pi session/history byte-equivalent. Both cases require zero fetch attempts and use bounded setup, bind, prompt/abort, idle, disposal, and filesystem cleanup waits.
...
- Exact interactive authorization: one deterministic confirmation, one consumed durable launch-grant audit, and replay rejection for the same request/tool-call ID.
- Full credential-free execution: public host `AgentSession` through production controller, QuickJS, managed journal, and durable Pi tool-result history.
